...
def index(request):
    ...
    file_submission_list = (
        FileSubmission.objects.filter(private=False)
        .order_by("-created_at")
        .annotate(submission_type=Value("file", output_field=CharField()))
    )
    # We do the same as above for link submissions.
    link_submission_list = (
        LinkSubmission.objects.filter(private=False)
        .order_by("-created_at")
        .annotate(submission_type=Value("link", output_field=CharField()))
    )
    ...
    return render(request, "main/index.html", {"submission_list": result_list})
